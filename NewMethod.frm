...
Private Sub Form_Load()
    ...
    y = 709
    Direction = 5
    TankBV = Array(35, 30, 28, 24, 18)
    GuyPickup = Array(2, 3, 4, 4.5, 4.5)
    TankSB = Array(2000, 1500, 1000, 750, 500)
    TankHits = Array(5, 10, 12, 15, 18)
    TankVel = Array(4, 6, 6, 6, 8)
    HeliSpeed = Array(14, 12, 10, 9, 8)
    HeliHits = Array(35, 30, 25, 20, 15)
    PBar.Max = HeliHits(Difficulty)
    '''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    For u = 0 To 15
        ...
        TankSpeed(u) = Int(Rnd * TankVel(Difficulty)) - TankVel(Difficulty) / 2
        Tank(u) = True
    Next
    AltStart.ProgBar1.Value = AltStart.ProgBar1.Value + 3
...
Public Function Run()
    ...
    While y > 600
        Choplifter.Refresh
        BitBlt Choplifter.hdc, 0, 0, Choplifter.ScaleWidth, Choplifter.ScaleHeight, BG, BGPosition, 0, vbSrcCopy
        BitBlt Choplifter.hdc, x, y, 135, 80, PlaneW, PX, 0, vbSrcAnd
        BitBlt Choplifter.hdc, x, y, 135, 80, PlaneB, PX, 0, vbSrcPaint
        y = y - 2
    Wend
    Do
        Choplifter.Refresh
        
        Up = GetAsyncKeyState(KeyUp)
        Down = GetAsyncKeyState(KeyDown)
        Right = GetAsyncKeyState(KeyRight)
        Lef = GetAsyncKeyState(KeyLeft)
        STP = GetAsyncKeyState(vbKeyEscape)
        Space = GetAsyncKeyState(KeyBombs)
        Shift = GetAsyncKeyState(KeyGuns)
        ...
        If Right <= -32767 Then
            ...
            If LRCounter = 2 Then
                ...
                LRCounter = 0
            End If
            If Direction < HeliSpeed(Difficulty) Then
        ...
        If Lef <= -32767 Then
            ...
            If LRCounter = 2 Then
                ...
                LRCounter = 0
            End If
            If Direction > -1 * HeliSpeed(Difficulty) Then
                Direction = Direction - 1
            End If
        End If
        ...
        If Guy(f) = True Then
            If GuyClimb(f) = True Then
                ...
                End If
            Else
                If (GuyX(f) < House(0).Left + House(0).Width And GuyX(f) > House(0).Left - 2) Or (GuyX(f) > House(1).Left And GuyX(f) < House(1).Left + House(1).Width) Or (GuyX(f) > House(2).Left And GuyX(f) < House(2).Left + House(2).Width) Or (GuyX(f) > House(3).Left And GuyX(f) < House(3).Left + House(3).Width) Or (GuyX(f) > House(4).Left And GuyX(f) < House(4).Left + House(4).Width) Or (GuyX(f) > House(5).Left And GuyX(f) < House(5).Left + House(5).Width) Or (GuyX(f) > House(6).Left And GuyX(f) < House(6).Left + House(6).Width) Then
                    Men (f)
                End If
                GuyX(f) = GuyX(f) - GuySpeed(f) - Direction
            End If
            If GuyX(f) + BGPosition < 5 And Guy(f) = True Then
                For k = 0 To 4
                    If Save(4 - k).Visible = True Then
                        Save(4 - k).Visible = False
                        Results.Label2.Caption = Results.Label2.Caption + 1
                        Exit For
                    End If
                Next
                Guy(f) = False
            ...
            If GuyY(f) < y + 45 And GuyClimb(f) = True Then
                ...
                Else
                    ...
                    Save(SavedInd).Visible = True
                    SavedInd = SavedInd + 1
                End If
            End If
            
            GuySpeed(f) = GuySpeed(f)
            If BGPosition + GuyX(f) < BGPosition + Choplifter.ScaleWidth And BGPosition + GuyX(f) >= BGPosition Then
                BitBlt Choplifter.hdc, GuyX(f), GuyY(f), 20, 30, GuyW, GuyP(f), 0, vbSrcAnd
                BitBlt Choplifter.hdc, GuyX(f), GuyY(f), 20, 30, GuyB, GuyP(f), 0, vbSrcPaint
                ...
                For E = 0 To 7
                    If Dirt(E) = True And DirtX(E) < GuyX(f) And DirtX(E) + 10 > GuyX(f) Then
                        ...
                        GraveX(f) = GuyX(f)
                        Talley (&HFF&)
                        Results.Label4.Caption = Results.Label4.Caption + 1
                    End If
                Next
            End If
        ElseIf Grave(f) = True Then
            BitBlt Choplifter.hdc, GraveX(f), 800, 15, 12, GraveBW, 15, 0, vbSrcAnd
            BitBlt Choplifter.hdc, GraveX(f), 800, 15, 12, GraveBW, 0, 0, vbSrcPaint
        ...
        Next
        If x < 140 And x > 36 And y > 645 And y < 709 And EndDirection = 0 Then
            KeyLock = True
            If y > 707 Then
                Label2.Visible = False
                For r = 0 To 50
                    If Saved(r) = True Then
                            Guy(r) = True
                            GuyX(r) = x + 65
                            Saved(r) = False
                            GuySpeed(r) = 2
            ...
            Else
                y = y + 1
            End If
            If SavedInd = 0 Then KeyLock = False
        End If
        ''''''''''''''
                     '
        ''''''       '
...
Public Function Bullets()
    While indb < 21
        ...
        If Bullet(indb).Visible = True Then
            ...
            If Bullet(indb).Left > Choplifter.ScaleWidth Or Bullet(indb).Left < 0 Then Bullet(indb).Visible = False
    ...
    For D = 0 To 7
        If Dirt(D) = True Then
            If DirtX(D) + BGPosition < 1059 Then
                BitBlt Choplifter.hdc, DirtX(D), 815, 10, 10, DirtW, DirtP(D), 0, vbSrcAnd
                BitBlt Choplifter.hdc, DirtX(D), 815, 10, 10, WaterB, DirtP(D), 0, vbSrcPaint
            ElseIf DirtX(D) + BGPosition < 1153 Then
                BitBlt Choplifter.hdc, DirtX(D), 815, 10, 10, DirtW, DirtP(D), 0, vbSrcAnd
                BitBlt Choplifter.hdc, DirtX(D), 815, 10, 10, SandB, DirtP(D), 0, vbSrcPaint
            Else
                BitBlt Choplifter.hdc, DirtX(D), 800, 10, 10, DirtW, DirtP(D), 0, vbSrcAnd
                BitBlt Choplifter.hdc, DirtX(D), 800, 10, 10, DirtB, DirtP(D), 0, vbSrcPaint
            End If
            If DirtP(D) = 10 Then
                DirtP(D) = 0
                Dirt(D) = False
...
Public Function Rope()
    If y >= 670 And x + BGPosition > 1059 Then
    ...
    If RopeP > 0 Then
        If RPlaneB = PlaneB Then
            BitBlt Choplifter.hdc, x + 100, y + 45, 20, RopeP, RopeW, 0, 0, vbSrcAnd
            BitBlt Choplifter.hdc, x + 100, y + 45, 20, RopeP, RopeB, 0, 0, vbSrcPaint
            If Direction >= -4 Then
                For G = 0 To 50
                    If BGPosition + GuyX(G) >= BGPosition And BGPosition + GuyX(G) <= BGPosition + Choplifter.ScaleWidth Then
                        If GuyX(G) > x + 100 And GuyX(G) < x + 120 And Guy(G) = True Then
                            If (Rnd * 5) > GuyPickup(Difficulty) Then GuyClimb(G) = True
                            Exit For
                        End If
                    End If
                Next
            End If
        Else
            BitBlt Choplifter.hdc, x + 35, y + 45, 20, RopeP, RopeW, 0, 0, vbSrcAnd
            BitBlt Choplifter.hdc, x + 35, y + 45, 20, RopeP, RopeB, 0, 0, vbSrcPaint
            For G = 0 To 50
                If BGPosition + GuyX(G) >= BGPosition And BGPosition + GuyX(G) <= BGPosition + Choplifter.ScaleWidth Then
...
Public Function Bombs()
    For t = 0 To 4
        If Bomb(t) = True Then
            BitBlt Choplifter.hdc, BarrelX(t), BarrelY(t), 20, 20, BarrelW, BarrelP(t), 0, vbSrcAnd
            BitBlt Choplifter.hdc, BarrelX(t), BarrelY(t), 20, 20, BarrelB, BarrelP(t), 0, vbSrcPaint
            BarrelX(t) = BarrelX(t) - Direction
            BarrelY(t) = BarrelY(t) * 1.04
            If BarrelY(t) < 0 Then Bomb(t) = False
            If BarrelY(t) > Choplifter.ScaleHeight - 80 Then
                Bomb(t) = False
                Boom(t) = True
                BoomX(t) = BarrelX(t)
            End If
        ...
        If Boom(t) = True Then
            If BoomX(t) + BGPosition > 1059 Then
                BitBlt Choplifter.hdc, BoomX(t), 790, 20, 20, BoomW, BoomP(t), 0, vbSrcAnd
                BitBlt Choplifter.hdc, BoomX(t), 790, 20, 20, BoomB, BoomP(t), 0, vbSrcPaint
                ...
                For j = 0 To 15
                    If (BoomP(t) <= 20 And Boom(t) = True And BoomX(t) <= TankX(j) + 40 And BoomX(t) + 20 >= TankX(j) And Tank(j) = True) Then
                        ...
                        If TankHit(j) = TankHits(Difficulty) Then
                            Tank(j) = False
                            TankHit(j) = 0
                        End If
                    End If
                Next
            Else
                BitBlt Choplifter.hdc, BoomX(t), 810, 20, 20, WBoomW, BoomP(t), 0, vbSrcAnd
                BitBlt Choplifter.hdc, BoomX(t), 810, 20, 20, WBoomB, BoomP(t), 0, vbSrcPaint
                BoomX(t) = BoomX(t) - Direction
                If SlowDownBoom(t) = 2 Then BoomP(t) = BoomP(t) + 20
                SlowDownBoom(t) = SlowDownBoom(t) + 1
    ...
    If Tank(L) = True Then
        For E = 0 To 7
            If Dirt(E) = True And DirtX(E) < TankX(L) + 40 And DirtX(E) + 10 > TankX(L) Then
                ...
                If TankHit(L) = TankHits(Difficulty) Then
                    Tank(L) = False
                    TankHit(L) = 0
                End If
            End If
        ...
        For i = 0 To 6
            If TankX(L) > House(i).Left And TankX(L) < House(i).Left + House(i).Width Then
                TankSpeed(L) = Int(Rnd * TankVel(Difficulty)) - TankVel(Difficulty) / 2
            End If
            If TankX(L) + BGPosition < 1153 Then TankSpeed(L) = 4
            If TankX(L) + BGPosition > 10282 Then TankSpeed(L) = -4
        Next
        If BGPosition + TankX(L) >= BGPosition And BGPosition + TankX(L) <= BGPosition + Choplifter.ScaleWidth Then
            If TankSlowDown(L) = 25 Then
                While TAind < 51
                    If TB(TAind).Visible = False Then
                        TB(TAind).Visible = True
                        TAngleX(TAind) = (x - TankX(L) + 20) / TankBV(Difficulty)
                        TAngleY(TAind) = (y - 790) / TankBV(Difficulty)
            ...
            BitBlt Choplifter.hdc, TankX(L), 785, 40, 30, TankW, 0, 0, vbSrcAnd
            BitBlt Choplifter.hdc, TankX(L), 785, 40, 30, TankB, 0, 0, vbSrcPaint
        End If
    Else
        ...
        If BGPosition + TankX(L) >= BGPosition And BGPosition + TankX(L) <= BGPosition + Choplifter.ScaleWidth Then
            ...
            BitBlt Choplifter.hdc, TankX(L), 785, 40, 30, TankB, 0, 0, vbSrcPaint
        End If
        If TankHit(L) = TankSB(Difficulty) Then
            Tank(L) = True
            TankHit(L) = 0
        End If
    End If
...
For c = 0 To 50
    If TB(c).Visible = True Then
        ...
        If TB(c).Left < 0 Or TB(c).Left > Choplifter.ScaleWidth Then TB(c).Visible = False
        If TB(c).Top < 0 Then
            TB(c).Visible = False
        End If
